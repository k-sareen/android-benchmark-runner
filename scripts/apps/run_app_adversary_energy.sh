...
if [[ "${device}" == "pixel7pro" ]]; then
    builds="art-mmtk-singlethread-immix-energy"
else
...
for build in ${builds}; do
    ...
    do_command cp ~/git/evaluation/configs/energy-stress/lbo-${build}-adv.yml ${tmp_file}
    echo -e "\nbenchmarks:\n  android-apps:" >> ${tmp_file}
    for bm in ${filtered_benchmark_names}; do
        echo "    - ${bm}" >> ${tmp_file}
    done

    echo -e ${GREEN}running -w "adb shell" runbms ~/git/evaluation/results/log ~/git/evaluation/configs/energy-stress/lbo-${build}.yml --reset-top-apps -i 10 -p `date +%F`-adversary-${orig_device}-${build} --invert-hfacs -s 1.5${NORM}
    running -w "adb shell" runbms ~/git/evaluation/results/log ${tmp_file} --reset-top-apps -i 10 -p `date +%F`-adversary-${orig_device}-${build} --invert-hfacs -s 1.5
