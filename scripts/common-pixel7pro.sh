pixel7pro_set_battery_charging_state () {
...
pixel7pro_get_memory_frequency_value_for_cpu () {
    check_device_and_fail "pixel7pro" "getting gs_memlat_devfreq"
...
pixel7pro_set_memory_frequency_value_for_cpu () {
    check_device_and_fail "pixel7pro" "setting gs_memlat_devfreq"
...
pixel7pro_get_memory_frequencies_and_governors () {
    check_device_and_fail "pixel7pro" "getting gs_memlat_devfreq"

    local cpus=${1}
    for i in ${cpus}; do
        pixel7pro_get_memory_frequency_value_for_cpu ${i} "available_frequencies" "Available frequencies"
        pixel7pro_get_memory_frequency_value_for_cpu ${i} "max_freq" "Current max frequency"
        pixel7pro_get_memory_frequency_value_for_cpu ${i} "available_governors" "Available governors"
        pixel7pro_get_memory_frequency_value_for_cpu ${i} "governor" "Current governor"
    done
}

pixel7pro_set_memory_frequencies_and_governors () {
    check_device_and_fail "pixel7pro" "setting gs_memlat_devfreq"
    ...
    local governor=${3}
    for i in ${cpus}; do
        pixel7pro_set_memory_frequency_value_for_cpu ${i} "min_freq" ${frequency} "min frequency"
        pixel7pro_set_memory_frequency_value_for_cpu ${i} "max_freq" ${frequency} "max frequency"
        pixel7pro_set_memory_frequency_value_for_cpu ${i} "governor" ${governor} "governor"
    done
}

pixel7pro_set_odpm_sampling_rate () {
    check_device_and_fail "pixel7pro" "setting ODPM"
...
pixel7pro_isolate_cores () {
    check_device_and_fail "pixel7pro" "isolating cores"
